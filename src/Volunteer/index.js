...
import { deleteVolunteer, getVolunteers } from '../Actions/volunteerActions';
...
function VolunteerList(props) {
  ...
  const Actions = [
    {
      element: <div className=' cursor-pointer flex gap-2'> <EyeIcon /> View Volunteer</div>,
      actionName: "view",
      action: commonAction
    },
    {
      element: <div className=' text-green-800 cursor-pointer flex gap-2'><PencilIcon /> Edit Volunteer </div>,
      actionName: "edit",
      action: commonAction
      // onClick: props.onClick,
    },
    {
      element: <div className=' text-red-800 cursor-pointer flex gap-2'><DeleteIcon /> Delete Volunteer</div>,
      actionName: "delete",
      action: commonAction
    }
  ]
      ...
      <div className="w-full h-full   text-gray-900">
        <main className=" mx-auto px-4 sm:px-6 lg:px-8 pt-4">
          ...
          {isDesktop && <div className="mt-6 ">
            <TableWrapper enableGlobalSearch={true} enableCreateRow={true} loading={loading} header={"Volenteers"} hideColums={['password', 'address', 'mobile' ,  role!='Head-Volunteer'? 'actions': 'random']}  ModalLoadedComponent={CreateUser} columns={columns} data={memoData} Actions={Actions} deleteMultipleRows={deleteMultipleRows} addNewElement={addNewElement} />
          </div>}
          {isTablet && <div className=" ">
            <TableWrapper enableGlobalSearch={true} enableCreateRow={true} loading={loading} header={"Volenteers "}  enableRowSelect={false} hideColums={['password', 'role', 'address', role!='Head-Volunteer'? 'actions': 'random']} ModalLoadedComponent={CreateUser} columns={columns} data={memoData} Actions={Actions} deleteMultipleRows={deleteMultipleRows} addNewElement={addNewElement} />
          </div>}
          {isMobile && <div className=" ">
            <TableWrapper enableGlobalSearch={true} enableCreateRow={true} loading={loading} header={"Volenteers"}  enableRowSelect={false} hideColums={['password', 'role', 'password', 'address',  role!='Head-Volunteer'&& 'actions']} ModalLoadedComponent={CreateUser} columns={columns} data={memoData} Actions={Actions} deleteMultipleRows={deleteMultipleRows} addNewElement={addNewElement} />
          </div>}
        </main>
      </div>
